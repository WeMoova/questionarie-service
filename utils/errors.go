...
func HandleRepositoryError(w http.ResponseWriter, err error) {
	...
	case contains(errMsg, "not active") || contains(errMsg, "no longer active") || contains(errMsg, "has no questions") || contains(errMsg, "cannot be empty") || contains(errMsg, "period has expired") || contains(errMsg, "already completed") || contains(errMsg, "already cancelled") || contains(errMsg, "cannot modify") || contains(errMsg, "cannot cancel") || contains(errMsg, "not all required"):
		ValidationError(w, errMsg)
	default:
